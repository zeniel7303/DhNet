...
`.claude/`(커스텀 명령·에이전트), `dev/`(작업 문서), AI 메모리를 `claude-workspace` private 레포로 관리한다.  
...
- `CLAUDE.md`는 `claude-workspace/CLAUDE.md`(공통 룰) + `claude-workspace/DhNet/CLAUDE.md`(프로젝트 룰)를 합쳐서 생성된다. 직접 편집하지 말고 workspace에서 수정할 것.
